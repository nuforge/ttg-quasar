    ...
    <q-card>
      ...
      <q-card-section>
        <q-form @submit.prevent="handleSubmit" class="q-gutter-lg">
          <!-- Event Title -->
          <q-input v-model="eventForm.formData.value.title" :label="`${t('eventTitle')} *`" outlined dense
            :rules="[eventForm.requiredRule]" lazy-rules aria-required="true" />

          <!-- Game Selection -->
          <q-select v-model="eventForm.formData.value.gameId" :options="eventForm.gameOptions.value"
            :label="`${t('game', 1)} *`" outlined dense emit-value map-options use-input fill-input hide-selected
            input-debounce="0" :rules="[eventForm.requiredRule]" lazy-rules aria-required="true"
          ...
          <div class="row q-col-gutter-lg">
            <div class="col-12 col-sm-4">
              <q-input v-model="eventForm.formData.value.date" :label="`${t('date')} *`" outlined dense type="date"
                :rules="[eventForm.requiredRule]" lazy-rules aria-required="true" />
            </div>
            <div class="col-12 col-sm-4">
              <q-input v-model="eventForm.formData.value.time" :label="`${t('startTime')} *`" outlined dense type="time"
                :rules="[eventForm.requiredRule]" lazy-rules aria-required="true" />
            </div>
            <div class="col-12 col-sm-4">
              <q-input v-model="eventForm.formData.value.endTime" :label="`${t('endTime')} *`" outlined dense
                type="time" :rules="[eventForm.requiredRule]" lazy-rules aria-required="true" />
            </div>
          </div>

          <!-- Location -->
          <q-input v-model="eventForm.formData.value.location" :label="`${t('location')} *`" outlined dense
            :rules="[eventForm.requiredRule]" lazy-rules aria-required="true">
          ...
          <div class="row q-col-gutter-lg">
            <div class="col-12 col-sm-6">
              <q-input v-model.number="eventForm.formData.value.minPlayers" :label="`${t('minPlayers')} *`" outlined
                dense type="number" min="1" :rules="[eventForm.requiredRule, eventForm.minPlayersRule]" lazy-rules
                aria-required="true" />
            </div>
            <div class="col-12 col-sm-6">
              <q-input v-model.number="eventForm.formData.value.maxPlayers" :label="`${t('maxPlayers')} *`" outlined
                dense type="number" min="1" :rules="[eventForm.requiredRule, eventForm.maxPlayersRule]" lazy-rules
                aria-required="true" />
            </div>
          </div>

          <!-- Description -->
          <q-input v-model="eventForm.formData.value.description" :label="t('description')" outlined type="textarea"
            rows="3" :hint="t('eventForm.descriptionHint')" />

          <!-- Notes (for organizers) -->
          <q-expansion-item icon="mdi-note-text-outline" :label="t('eventForm.organizerNotes')" caption="Optional">
            <q-card-section class="q-pt-none">
              <q-input v-model="eventForm.formData.value.notes" outlined type="textarea" rows="2"
                :hint="t('eventForm.notesHint')" />
            </q-card-section>
          </q-expansion-item>

          <!-- Google Calendar Sync (new events only) -->
          <div v-if="!eventForm.isEditMode.value" class="q-mt-md">
            <q-toggle v-model="eventForm.formData.value.syncToGoogleCalendar" :label="t('eventForm.syncToCalendar')"
              color="primary" />
      ...
      <q-card-actions align="right" class="q-pa-md">
        <q-btn flat color="grey-7" :label="t('cancel')" @click="dialogVisible = false"
          :disable="eventForm.loading.value" />
        <q-btn color="primary" :label="submitLabel" @click="handleSubmit" :loading="eventForm.loading.value"
          :disable="!eventForm.isFormValid.value"
          :icon="eventForm.isEditMode.value ? 'mdi-content-save' : 'mdi-calendar-plus'" />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
