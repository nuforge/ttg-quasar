...
const statusColor = computed(() => {
    if (confirmedCount.value >= maxPlayers.value) return 'green';
    if (confirmedCount.value >= maxPlayers.value * 0.7) return 'orange';
    return 'blue';
});
</script>
...
<template>
    <div class="event-rsvp-summary">
        ...
        <div class="rsvp-counts q-mb-sm">
            <q-chip :color="statusColor" icon="mdi-account-check" :label="`${confirmedCount} / ${maxPlayers}`"
                size="md">
